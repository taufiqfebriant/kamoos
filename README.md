...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create kamoos-5d9b
  fly apps create kamoos-5d9b-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app kamoos-5d9b
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app kamoos-5d9b-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'kamoos-5d9b-staging' does not match app name in config file 'kamoos-5d9b'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name kamoos-5d9b-db
  fly postgres attach --postgres-app kamoos-5d9b-db --app kamoos-5d9b

  fly postgres create --name kamoos-5d9b-staging-db
  fly postgres attach --postgres-app kamoos-5d9b-staging-db --app kamoos-5d9b-staging
  ```
